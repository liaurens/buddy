...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Load AI config from user settings
    ...
    const provider = aiConfig['ai_aiProvider'] || 'anthropic'
    const key = aiConfig['ai_aiApiKey']
    const rawModel = aiConfig['ai_aiModel']
    const model = (rawModel && rawModel !== 'null') ? rawModel : undefined

    if (!key) {
      return jsonResponse(
        { error: 'No AI API key configured. Set one in Settings.' },
        400
      )
    }
