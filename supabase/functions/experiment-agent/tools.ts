...
export const EXPERIMENT_TOOLS: ToolSchema[] = [
    {
        name: 'get_experiment',
        description:
            'Get the full details of the current experiment including custom metrics, phases, status, hypothesis, and tags.',
        input_schema: {
            type: 'object',
            properties: {},
        },
    ...
    },
    {
        ...
        description:
            'Get recent check-in entries for the current experiment, grouped by date and metric. Useful for analysis and summaries.',
        input_schema: {
            type: 'object',
            properties: {
                days: {
    ...
    },
    {
        name: 'update_metrics',
        description:
            'Replace the custom_metrics array on the current experiment. Use this to add, remove, or modify experiment-specific check-in metrics. Provide the FULL desired metrics array — it replaces the existing one.',
        input_schema: {
            type: 'object',
            properties: {
                metrics: {
                    type: 'array',
                    description:
                        'Full array of metrics. Each metric: {id, name, emoji, type (rating|number|boolean|text), unit?, min?, max?, required?, description?}',
                    items: {
                        type: 'object',
                        properties: {
                            id: { type: 'string' },
    ...
    },
    {
        name: 'update_phases',
        description:
            'Replace the phases array on the current experiment. Provide the FULL array — it replaces existing phases.',
        input_schema: {
            type: 'object',
            properties: {
                phases: {
                    type: 'array',
                    description:
                        'Full array of phases. Each phase: {id, name, startDate (YYYY-MM-DD), endDate? (YYYY-MM-DD), description?, order}',
                    items: {
                        type: 'object',
                        properties: {
                            id: { type: 'string' },
    ...
    },
    {
        ...
        description:
            'Update top-level fields on the current experiment: name, hypothesis, description, status, tags, checkinSchedule.',
        input_schema: {
            type: 'object',
            properties: {
                name: { type: 'string' },
    ...
    },
    {
        ...
        description:
            'Compute summary statistics (mean, count, trend) for each custom metric, broken down by phase. Use this when the user asks for analysis or interpretation.',
        input_schema: {
            type: 'object',
            properties: {},
        },
...
async function getExperiment(ctx: ToolContext): Promise<ToolExecutionResult> {
    ...
    if (error || !data) {
        return {
            ok: false,
            summary: `Could not load experiment: ${error?.message ?? 'not found'}`,
        };
    }
...
async function updateMetrics(
    ...
    return {
        ok: true,
        summary: `Updated metrics (${metrics.length} total)`,
...
async function updatePhases(
    ...
    return {
        ok: true,
        summary: `Updated phases (${phases.length} total)`,
...
async function updateExperiment(
    ...
    return {
        ok: true,
        summary: `Updated experiment fields: ${Object.keys(update)
