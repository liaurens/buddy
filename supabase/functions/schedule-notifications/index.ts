...
async function loadDeliveryPrefs(
    ...
    try {
        ...
        for (const row of data || []) {
            ...
            switch (row.key) {
                case 'notifications_quietHoursEnabled':
                    prefs.quietHoursEnabled = value !== false;
                    break;
                case 'notifications_quietHoursStart':
                    prefs.quietHoursStart = String(value);
                    break;
                case 'notifications_quietHoursEnd':
                    prefs.quietHoursEnd = String(value);
                    break;
                ...
                case 'account_timezone':
                    if (typeof value === 'string' && value) prefs.timezone = value;
                    break;
            }
        }
    } catch (error) {
    ...
    try {
        ...
        for (const [userId, userNotifications] of Object.entries(notificationsByUser)) {
            ...
            if (
                ...
                inQuietHours(localMins, prefs.quietHoursStart, prefs.quietHoursEnd)
            ) {
            ...
            for (const notification of userNotifications) {
                ...
                await supabase
                    ...
                    .update({
                        ...
                        sent_at: new Date().toISOString(),
                        error_message: notificationSent
                            ? null
                            : 'Failed to send to any subscription',
                    })
                    .eq('id', notification.id);
