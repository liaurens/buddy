...
async function tryEnqueue(supabase: any, args: EnqueueArgs): Promise<boolean> {
...
async function loadSettings(supabase: any, userId: string): Promise<NotificationsSettings> {
...
async function recentSentCount(supabase: any, userId: string, sinceMinutes: number): Promise<number> {
...
async function scanUser(supabase: any, userId: string): Promise<number> {
  ...
  if (settings.offTrackMissedRoutines) {
    const routines: Array<{ key: 'morning' | 'midday' | 'night'; enabled?: boolean; time?: string; label: string; route: string }> = [
      { key: 'morning', enabled: settings.morningEnabled, time: settings.morningTime, label: 'morning routine', route: 'today' },
      { key: 'midday', enabled: settings.middayEnabled, time: settings.middayTime, label: 'midday replan', route: 'today' },
      { key: 'night', enabled: settings.nightEnabled, time: settings.nightTime, label: 'night reflection', route: 'reflection' },
    ];
  ...
  if (settings.offTrackIdle && now.getHours() >= 9 && now.getHours() <= 21) {
    ...
    const [{ count: c1 }, { count: c2 }, { count: c3 }] = await Promise.all([
      supabase.from('todos').select('id', { count: 'exact', head: true }).eq('user_id', userId).gte('created_at', sixHoursAgo),
      supabase.from('entries').select('id', { count: 'exact', head: true }).eq('user_id', userId).gte('created_at', sixHoursAgo),
      supabase.from('smart_notes').select('id', { count: 'exact', head: true }).eq('user_id', userId).gte('created_at', sixHoursAgo),
    ]);
...
serve(async (req) => {
  ...
  try {
    ...
    const uniqueUsers = Array.from(new Set((subs || []).map((s: { user_id: string }) => s.user_id)));
    let totalEnqueued = 0;
    ...
    return new Response(
      JSON.stringify({ users: uniqueUsers.length, enqueued: totalEnqueued, timestamp: new Date().toISOString() }),
