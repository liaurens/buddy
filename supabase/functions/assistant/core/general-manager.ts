...
function buildAgentResponse(
    ...
    const actionTaken = loop.finalText
        ? loop.finalText
        : loop.steps
              .map((s) => `${s.result.success ? '✓' : '✗'} ${s.result.action_taken}`)
              .join('\n');
...
export async function handleRequest(
                    ...
                    if (isEmptyLoop && context.aiConfig?.key) {
                        try {
                            ...
                            const top: ClarifyCandidate = {
                                ...
                                label:
                                    classified.routed.action.split('.')[0] ||
                                    classified.routed.action,
                            };
                            const seen = new Set<string>([`${top.domain}:${top.intent}`]);
                            const merged = [
                                top,
                                ...classified.alternatives.filter((a) => {
                    ...
                    logInteraction(
                        {
                            ...
                            toolId: undefined,
                            routingMethod: 'ai',
                            errorDetails: response.success
                                ? undefined
                                : { stoppedReason: loopResult.stoppedReason },
                            aiCalls: aiCalls.toJSON(),
                            processingSteps: tracker.getSteps() as unknown as Array<
    ...
    const toolId = manager.tools.find((t) => t.actions.some((a) => a.action === routed.action))?.id;
