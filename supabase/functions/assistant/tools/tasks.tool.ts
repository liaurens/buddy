...
// deno-lint-ignore no-explicit-any
async function findOpenTask(title: string, userId: string, supabase: any) {
...
async function handleSetReminder(params: Record<string, unknown>, context: AgentContext): Promise<ToolResult> {
  const target = ((params.task as string) || (params.title as string) || '').trim()
  if (!target) {
    return { success: false, action_taken: 'Provide a task title to set a reminder on.', data: {} }
  }
  // deno-lint-ignore no-explicit-any
  const supabase = context.supabase as any
  const task = await findOpenTask(target, context.userId, supabase)
  if (!task) {
    return { success: false, action_taken: `Could not find open task matching "${target}"`, data: {} }
  }
  ...
  if (!at && offsetMinutes === null) {
    return {
      success: false,
      action_taken: 'Provide either at (ISO 8601 timestamp) or offset_minutes (minutes before due_date).',
      data: {},
    }
  }
  ...
  if (error) {
    return { success: false, action_taken: 'Failed to set reminder', data: { error: error.message } }
  }
...
async function handleCancelReminder(params: Record<string, unknown>, context: AgentContext): Promise<ToolResult> {
  const target = ((params.task as string) || (params.title as string) || '').trim()
  if (!target) {
    return { success: false, action_taken: 'Provide a task to cancel the reminder on.', data: {} }
  }
  // deno-lint-ignore no-explicit-any
  const supabase = context.supabase as any
  const task = await findOpenTask(target, context.userId, supabase)
  ...
  const { error } = await supabase
    ...
    .eq('user_id', context.userId)
  if (error) {
    return { success: false, action_taken: 'Failed to cancel reminder', data: { error: error.message } }
  }
  return {
    success: true,
...
async function handleListReminders(_params: Record<string, unknown>, context: AgentContext): Promise<ToolResult> {
  // deno-lint-ignore no-explicit-any
  const supabase = context.supabase as any
  const { data: tasks, error } = await supabase
    ...
    .limit(20)
  if (error) {
    return { success: false, action_taken: 'Failed to list reminders', data: { error: error.message } }
  }
  const count = tasks?.length ?? 0
  if (count === 0) {
    return { success: true, action_taken: 'No active reminders.', data: { reminders: [], count: 0 } }
  }
  // deno-lint-ignore no-explicit-any
...
export const tasksTool: ToolDefinition = {
  ...
  actions: [
    {
      action: 'task.create',
      description: 'Create a new todo task. Example: { title: "call dentist", due_date: "2026-05-14T10:00:00", priority: "high" }. Use task_reminder_set to add a reminder to an EXISTING task — don\'t set is_reminder=true unless the user explicitly wants a reminder-style task with no further reminders.',
      inputSchema: {
        type: 'object',
        properties: {
          title: { type: 'string', description: 'Task title (required).' },
          due_date: { type: 'string', format: 'date-time', description: 'ISO 8601 deadline. Interpreted as the user\'s local time when no offset is given.' },
          priority: { type: 'string', enum: ['low', 'normal', 'high'], description: 'Task priority' },
          is_reminder: { type: 'boolean', description: 'True if the task is a reminder rather than a regular todo. Rarely needed — prefer task_reminder_set on an existing task.' },
        },
        required: ['title'],
      },
    ...
    },
    {
      ...
      description: 'Set a reminder on an existing open task. Provide either `at` (ISO 8601 absolute time) OR `offset_minutes` (minutes before due_date). Cadence defaults to "once".',
      inputSchema: {
        type: 'object',
        properties: {
          task: { type: 'string', description: 'Task title to fuzzy-match among open todos.' },
          at: { type: 'string', format: 'date-time', description: 'Absolute ISO 8601 timestamp for the reminder.' },
          offset_minutes: { type: 'integer', description: 'Minutes before the task\'s due_date.' },
          cadence: { type: 'string', enum: ['once', 'daily', 'weekly'], description: 'How often the reminder repeats.' },
        },
        required: ['task'],
      },
