...
async function resolveClass(
    ...
    const matches =
        exact.length > 0 ? exact : list.filter((c) => c.name.toLowerCase().includes(needle));
...
async function handleClassCreate(
    ...
    if (error) {
        return {
            success: false,
            action_taken: 'Failed to create class',
...
async function handleClassList(
    ...
    let q = sb
        .from('classes')
        .select('id, name, instructor, term, color, archived')
        .eq('user_id', context.userId)
    ...
    if (error) {
        return {
            success: false,
            action_taken: 'Failed to list classes',
            data: { error: error.message },
        };
    }
    const classes = data ?? [];
...
async function handleAssignmentCreate(
    ...
    if (Number.isNaN(deadlineDate.getTime())) {
        return {
            success: false,
            action_taken: `Invalid deadline ISO timestamp: ${deadline}`,
            data: {},
        };
    }
    ...
    const { data: assignment, error } = await sb
        ...
        .insert({
            ...
            status: 'pending',
            estimated_minutes:
                typeof params.estimated_minutes === 'number' ? params.estimated_minutes : null,
        })
        .select()
        .single();
    ...
    if (error) {
        return {
            success: false,
            action_taken: 'Failed to create assignment',
            data: { error: error.message },
        };
    }
    ...
    const reminderTimeOfDay =
        typeof params.reminder_time_of_day === 'string' ? params.reminder_time_of_day : '';
...
async function handleAssignmentList(
    ...
    if (error) {
        return {
            success: false,
            action_taken: 'Failed to list assignments',
            data: { error: error.message },
        };
    }
...
async function handleAssignmentComplete(
    ...
    if (!assignmentId) {
        return {
            success: false,
            action_taken: 'Provide assignment_id or a title to match.',
            data: {},
        };
    }
    ...
    if (error) {
        return {
            success: false,
            action_taken: 'Failed to update assignment',
            data: { error: error.message },
        };
    }
    return {
...
async function handleSessionList(
    ...
    if (error) {
        return {
            success: false,
            action_taken: 'Failed to list sessions',
            data: { error: error.message },
        };
    }
    return {
...
async function handleSessionCreate(
    ...
    if (!Number.isInteger(dayOfWeek) || dayOfWeek < 0 || dayOfWeek > 6 || !startTime || !endTime) {
        return {
            success: false,
            action_taken: 'day_of_week (0-6), start_time, end_time are required',
            data: {},
        };
    }
    const resolved = await resolveClass(
    ...
    if (error) {
        return {
            success: false,
            action_taken: 'Failed to create class session',
            data: { error: error.message },
        };
    }
    return {
...
export const schoolTool: ToolDefinition = {
    ...
    actions: [
        ...
        },
        {
            ...
            description:
                'Create a school assignment with a deadline. If reminder_days_before / reminder_hours_before is set, also schedules a reminder relative to the deadline.',
            inputSchema: {
                type: 'object',
                properties: {
                    ...
                    class_name: {
                        type: 'string',
                        description: 'Class name to fuzzy-match if class_id is unknown',
                    },
                    description: { type: 'string', description: 'Optional details' },
                    estimated_minutes: {
                        type: 'integer',
                        description: 'Estimated work time in minutes',
                    },
                    reminder_days_before: {
                        type: 'integer',
                        description: 'Schedule a reminder N days before the deadline',
                    },
                    reminder_hours_before: {
                        type: 'integer',
                        description: 'Schedule a reminder N hours before the deadline',
                    },
                    reminder_time_of_day: {
                        type: 'string',
                        description: 'Override the reminder time-of-day in HH:MM (e.g. "09:00")',
                    },
                },
        ...
        },
        {
            ...
            description:
                'List school assignments, optionally filtered by class, status, or upcoming window.',
            inputSchema: {
                type: 'object',
                properties: {
                    ...
                    days_ahead: {
                        type: 'integer',
                        description: 'Only show assignments due within the next N days',
                    },
                },
            },
        ...
        },
        {
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    title: {
                        type: 'string',
                        description: 'Fuzzy-match a title if assignment_id is unknown',
                    },
                    status: {
                        type: 'string',
                        enum: ['pending', 'in_progress', 'submitted', 'graded'],
