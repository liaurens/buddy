...
async function handleScheduleNotification(
    ...
    const scheduledForParam =
        typeof params.scheduled_for === 'string' ? params.scheduled_for : null;
    if (messageParam && scheduledForParam) {
        const date = new Date(scheduledForParam);
        if (Number.isNaN(date.getTime())) {
            return {
                success: false,
                action_taken: 'Invalid scheduled_for ISO timestamp',
                data: {},
            };
        }
    ...
    if (!time) {
        return {
            success: false,
            action_taken:
                'Reminder needs a time. Try "/remind 14:00 call dentist" or "in 2 uur opbellen".',
...
async function handleScheduleRelative(
    ...
    if (Number.isNaN(anchorDate.getTime())) {
        return {
            success: false,
            action_taken: `Invalid anchor ISO timestamp: ${anchor}`,
            data: {},
        };
    }
    ...
    const totalMs = offsetDays * 86_400_000 + offsetHours * 3_600_000 + offsetMinutes * 60_000;
...
export const notificationsTool: ToolDefinition = {
    ...
    actions: [
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    scheduled_for: {
                        type: 'string',
                        format: 'date-time',
                        description: 'ISO 8601 timestamp when the reminder fires',
                    },
                    tool_category: {
                        type: 'string',
                        enum: [...VALID_TOOL_CATEGORIES],
                        description:
                            'Which app section this reminder relates to (default: planning)',
                    },
                },
        ...
        },
        {
            ...
            description:
                'Schedule a reminder relative to a future anchor time (e.g. "1 week before deadline"). scheduled_for = anchor − offsets.',
            inputSchema: {
                type: 'object',
                properties: {
                    message: { type: 'string', description: 'What the reminder should say' },
                    anchor: {
                        type: 'string',
                        format: 'date-time',
                        description: 'The reference timestamp the reminder is offset from',
                    },
                    offset_days: {
                        type: 'integer',
                        description: 'Days before anchor (e.g. 7 for "a week before")',
                    },
    ...
    commands: [
        {
            command: '/remind',
            action: 'notification.schedule',
            description: 'Set a reminder: /remind 14:00 call dentist',
            primary: true,
        },
    ],
