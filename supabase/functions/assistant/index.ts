...
Deno.serve(async (req) => {
    ...
    try {
        ...
        // Shortcut tokens are for natural-language capture commands. Direct
        // actions include credential management and require a user JWT.
        if (body.api_key && isDirect) {
            return jsonResponse(
                {
                    success: false,
                    error: 'Direct actions require account authentication',
                    intent: body.action ?? 'unknown',
                    action_taken: '',
                    data: {},
                },
                403,
        ...
        // Credentials are service-role-only and never returned to browser code.
        const { data: credential } = await supabase
            .from('ai_credentials')
            .select('provider, api_key, model')
            .eq('user_id', userId)
            .maybeSingle();
        const resolvedAiConfig = credential?.api_key
            ? {
                  key: credential.api_key as string,
                  provider: credential.provider as string,
                  model: (credential.model as string | null) ?? undefined,
              }
            : undefined;
