        ...
        {exportedMarkdown ? (
          <div className="flex-1 flex flex-col min-h-0 bg-gray-50 p-6 overflow-hidden">
             <div className="flex items-center justify-between mb-4">
               <span className="text-sm text-gray-500 border border-gray-300 bg-gray-100 px-3 py-1 rounded inline-block">Markdown Viewer</span>
                ...
                <table className="min-w-full divide-y divide-gray-200 text-sm text-left text-gray-600 custom-table-layout">
                  <thead className="bg-gray-50 sticky top-0 z-10 shadow-sm">
                    <tr>
                      ...
                      <th scope="col" className="px-6 py-3 font-medium text-gray-500 uppercase tracking-wider w-1/3">Description</th>
                      <th scope="col" className="px-6 py-3 font-medium text-gray-500 uppercase tracking-wider">Date</th>
                      <th scope="col" className="px-6 py-3 font-medium text-gray-500 uppercase tracking-wider">Element Selector</th>
                      <th scope="col" className="px-6 py-3 font-medium text-gray-500 uppercase tracking-wider text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {reports.map((report) => (
                      <tr key={report.id} onClick={() => handleSelectOne(report.id!)} className="hover:bg-indigo-50/50 transition-colors cursor-pointer">
                        <td className="px-6 py-4 whitespace-nowrap w-12 text-center" onClick={(e) => e.stopPropagation()}>
                           <input 
                             type="checkbox" 
                             className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer"
                        ...
                        <td className="px-6 py-4 whitespace-nowrap flex items-center gap-2 font-medium capitalize">
                          {getIcon(report.type)}
                          {report.type === 'feature' ? 'Change Req' : report.type}
                        </td>
                        <td className="px-6 py-4" title={report.description}>
                        ...
                        <td className="px-6 py-4">
                          {report.selector ? (
                             <div className="text-xs font-mono bg-slate-100 text-slate-600 px-2 py-1 rounded max-w-[150px] truncate" title={report.selector}>
                               {report.selector}
                             </div>
                          ) : (
                             <span className="text-gray-300 italic text-xs">No element attached</span>
                          )}
                        </td>
