...
import { ChevronLeft, ChevronRight, Mail, Activity, Coffee, Check, Settings, Plus, Clock, RefreshCw } from 'lucide-react';
                ...
                <div className="space-y-4">
                    ...
                    {items.untimedPicks.length > 0 && (
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-2">
                            <h3 className="font-semibold text-slate-900 text-sm">Give your picks a time (optional)</h3>
                            <p className="text-xs text-slate-500">
                                Add a time and a pick lands on the timeline next to your calendar events.
                            </p>
                            <ul className="space-y-2 pt-1">
                                {items.untimedPicks.map(task => (
                                    <li key={task.id} className="flex items-center gap-2">
                                        ...
                                        <input
                                            type="time"
                                            onChange={e => handleSetPickTime(task, e.target.value)}
