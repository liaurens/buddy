    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      <div className="p-3 border-b border-slate-100 bg-orange-50/50">
        <div className="flex gap-2">
          ...
          >
            {hrRunning ? <RefreshCw size={12} className="animate-spin" /> : <Play size={12} />}
            Run HR Agent
          </button>
          <button
        ...
        {agentResult && (
          <p className="text-xs text-slate-600 mt-2 bg-white rounded px-2 py-1">{agentResult}</p>
        )}
      </div>
      ...
      <div className="max-h-96 overflow-y-auto">
          ...
          <div className="divide-y divide-slate-50">
            {errorLogs.length === 0 ? (
              ...
              <div key={log.id} className="text-xs">
                ...
                >
                  {expandedId === log.id ? <ChevronDown size={12} className="mt-0.5 shrink-0" /> : <ChevronRight size={12} className="mt-0.5 shrink-0" />}
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${
                        log.error_type === 'ai_error' ? 'bg-purple-100 text-purple-700' :
                        log.error_type === 'execution_error' ? 'bg-red-100 text-red-700' :
                        log.error_type === 'routing_error' ? 'bg-orange-100 text-orange-700' :
                        'bg-slate-100 text-slate-600'
                      }`}>{log.error_type}</span>
                      <span className="text-slate-400">{log.step}</span>
                      {log.domain && <span className="text-slate-300">| {log.domain}</span>}
                    </div>
                    <p className="text-slate-600 truncate mt-0.5">{log.error_message}</p>
                  </div>
                  <span className="text-slate-300 shrink-0">{formatTime(log.created_at)}</span>
                </button>
                {expandedId === log.id && (
                  <div className="px-8 pb-3 space-y-1 text-slate-500">
                    <p><strong>Input:</strong> {log.input}</p>
                    {log.intent && <p><strong>Intent:</strong> {log.intent}</p>}
                    {log.ai_provider && <p><strong>Provider:</strong> {log.ai_provider} {log.ai_model && `/ ${log.ai_model}`}</p>}
                    {log.error_stack && (
                      <pre className="mt-1 p-2 bg-slate-50 rounded text-[10px] overflow-x-auto whitespace-pre-wrap">{log.error_stack}</pre>
                    )}
                    {log.context && Object.keys(log.context).length > 0 && (
                      <pre className="mt-1 p-2 bg-slate-50 rounded text-[10px] overflow-x-auto">{JSON.stringify(log.context, null, 2)}</pre>
                    )}
                  </div>
                )}
              </div>
        ...
        {activeTab === 'findings' && (
          <div className="divide-y divide-slate-50">
            {findings.length === 0 ? (
              <p className="p-4 text-sm text-slate-400 text-center">No findings yet. Run the HR Agent to analyze logs.</p>
            ) : findings.map(f => (
              <div key={f.id} className="text-xs">
                <button
                  ...
                  className="w-full px-3 py-2 flex items-start gap-2 hover:bg-slate-50 text-left"
                >
                  {expandedId === f.id ? <ChevronDown size={12} className="mt-0.5 shrink-0" /> : <ChevronRight size={12} className="mt-0.5 shrink-0" />}
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${
                      ...
                      <span className={`px-1.5 py-0.5 rounded text-[10px] ${
                        f.status === 'new' ? 'bg-blue-50 text-blue-600' :
                        f.status === 'applied' ? 'bg-green-50 text-green-600' :
                        'bg-slate-50 text-slate-500'
                      }`}>{f.status}</span>
                    </div>
                    <p className="text-slate-600 truncate mt-0.5">{f.data?.summary || 'No summary'}</p>
                  </div>
                  <span className="text-slate-300 shrink-0">{formatTime(f.created_at)}</span>
                </button>
                {expandedId === f.id && (
                  <div className="px-8 pb-3">
                    <pre className="p-2 bg-slate-50 rounded text-[10px] text-slate-500 overflow-x-auto whitespace-pre-wrap">{JSON.stringify(f.data, null, 2)}</pre>
                  </div>
                )}
              </div>
            ))}
        ...
        {activeTab === 'rules' && (
          <div className="divide-y divide-slate-50">
            {rules.length === 0 ? (
              <p className="p-4 text-sm text-slate-400 text-center">No dynamic rules yet. The Trainer creates these from findings.</p>
            ) : rules.map(r => (
              <div key={r.id} className="px-3 py-2 flex items-center gap-2 text-xs hover:bg-slate-50">
                <button
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <code className="px-1 py-0.5 bg-slate-100 rounded text-[10px] font-mono">{r.pattern}</code>
        ...
        {activeTab === 'learnings' && (
          <div className="divide-y divide-slate-50">
            {learnings.length === 0 ? (
              <p className="p-4 text-sm text-slate-400 text-center">No learnings yet.</p>
            ) : learnings.map(l => (
              <div key={l.id} className="text-xs">
                <button
                  ...
                  className="w-full px-3 py-2 flex items-start gap-2 hover:bg-slate-50 text-left"
                >
                  {expandedId === l.id ? <ChevronDown size={12} className="mt-0.5 shrink-0" /> : <ChevronRight size={12} className="mt-0.5 shrink-0" />}
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${
                        l.type === 'new_rule' ? 'bg-green-100 text-green-700' :
                        l.type === 'correction' ? 'bg-red-100 text-red-700' :
                        l.type === 'behavior' ? 'bg-purple-100 text-purple-700' :
                        'bg-slate-100 text-slate-600'
                      }`}>{l.type}</span>
                      <span className={`text-[10px] ${l.active ? 'text-green-500' : 'text-slate-300'}`}>
                        {l.active ? 'active' : 'inactive'}
                      </span>
                    </div>
                    <p className="text-slate-600 truncate mt-0.5">
                      {l.content?.finding_type || l.content?.source || l.type}
                    </p>
                  </div>
                  <span className="text-slate-300 shrink-0">{formatTime(l.created_at)}</span>
                </button>
                {expandedId === l.id && (
                  <div className="px-8 pb-3">
                    <pre className="p-2 bg-slate-50 rounded text-[10px] text-slate-500 overflow-x-auto whitespace-pre-wrap">{JSON.stringify(l.content, null, 2)}</pre>
                  </div>
                )}
              </div>
            ))}
