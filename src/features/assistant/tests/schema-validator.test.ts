...
function validate(value: unknown, schema: JsonSchema | undefined, path = ''): ValidateResult {
    ...
    if (schema.required) {
      for (const key of schema.required) {
        if (!(key in obj) || obj[key] === undefined || obj[key] === null) {
          return { ok: false, error: `missing required field "${key}"${path ? ` at ${path}` : ''}`, field: path ? `${path}.${key}` : key }
        }
      }
    }
  ...
  if (t === 'array') {
    if (!Array.isArray(value)) return { ok: false, error: `expected array at ${path || 'root'}`, field: path }
    if (schema.items) {
  ...
  if (schema.enum && !schema.enum.includes(value as string | number)) {
    return { ok: false, error: `value at ${path || 'root'} must be one of ${schema.enum.join(', ')}`, field: path }
  }
  return { ok: true }
}
