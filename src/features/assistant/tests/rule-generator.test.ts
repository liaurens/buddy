...
async function generateRules(finding: Finding, supabase: MockSupabase): Promise<GenerateResult> {
    switch (finding.type) {
...
async function handleUnmatchedPattern(
    ...
    const { error } = await supabase.from('assistant_rules').insert({
...
async function handleErrorCluster(
    ...
    const data = finding.data as Record<string, unknown> | undefined;
    await supabase.from('assistant_learnings').insert({
        user_id: finding.user_id,
        type: 'correction',
        content: {
            source: 'hr_agent',
...
describe('extractCommonKeywords', () => {
    ...
    it('filters out stop words', () => {
        const examples = ['the quick brown fox', 'a quick red fox', 'the quick blue fox'];
        const keywords = extractCommonKeywords(examples);
    ...
    it('filters out short words (<=2 chars)', () => {
        const examples = ['go to gym', 'go to pool', 'go to park'];
        const keywords = extractCommonKeywords(examples);
...
describe('generateRules', () => {
    ...
    it('skips when a matching rule already exists', async () => {
        const selectResults = new Map([['assistant_rules', [{ id: 'existing-rule' }]]]);
    ...
    it('uses proposed_rule domain/action when available', async () => {
        ...
        const finding = {
            ...
            data: {
                ...
                examples: ['log my workout today', 'record workout session', 'save workout data'],
            },
        };
        const result = await generateRules(finding, supabase);
        expect(result.rulesCreated).toBe(1);
