...
function parseDateExpression(input: string): string | null {
  ...
  const dayMap: Record<string, number> = {
    monday: 1, maandag: 1,
    tuesday: 2, dinsdag: 2,
    wednesday: 3, woensdag: 3,
    thursday: 4, donderdag: 4,
    friday: 5, vrijdag: 5,
  ...
  for (const [name, month] of Object.entries(monthNames)) {
    const match = lower.match(new RegExp(`\\b(\\d{1,2})\\s+${name}\\b|\\b${name}\\s+(\\d{1,2})\\b`))
    if (match) {
...
describe('parseDateExpression', () => {
  ...
  describe('relative dates (English)', () => {
    ...
    it('parses "day after tomorrow" (known: "tomorrow" matches first)', () => {
      ...
      expect(parseDateExpression('finish day after tomorrow')).toBe(toDateString(addDays(FIXED_DATE, 1)))
    })
  ...
  describe('relative dates (Dutch)', () => {
    ...
    it('parses "overmorgen"', () => {
      expect(parseDateExpression('klaar overmorgen')).toBe(toDateString(addDays(FIXED_DATE, 2)))
    })
  ...
  describe('day names', () => {
    ...
    it('parses "monday" as next Monday (+5 days)', () => {
      expect(parseDateExpression('meeting monday')).toBe(toDateString(addDays(FIXED_DATE, 5)))
    })

    it('parses "friday" as this Friday (+2 days)', () => {
      expect(parseDateExpression('deadline friday')).toBe(toDateString(addDays(FIXED_DATE, 2)))
    })

    it('parses "wednesday" as next Wednesday (+7, same day)', () => {
      expect(parseDateExpression('meet on wednesday')).toBe(toDateString(addDays(FIXED_DATE, 7)))
    })

    it('parses Dutch "maandag"', () => {
      expect(parseDateExpression('vergadering maandag')).toBe(toDateString(addDays(FIXED_DATE, 5)))
    })

    it('parses Dutch "vrijdag"', () => {
      expect(parseDateExpression('deadline vrijdag')).toBe(toDateString(addDays(FIXED_DATE, 2)))
    })
  ...
  describe('relative "in X days"', () => {
    it('parses "in 3 days"', () => {
      expect(parseDateExpression('finish in 3 days')).toBe(toDateString(addDays(FIXED_DATE, 3)))
    })
