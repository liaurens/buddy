...
function analyzeUnmatchedPatterns(logs: LogEntry[]): Finding[] {
  const aiRouted = logs.filter(l =>
    ...
    (l.detection_method === 'rule' && l.detected_intent === 'note.create' && !(l.input as string)?.startsWith('/'))
  )
...
function analyzeSlowRoutes(logs: LogEntry[]): Finding[] {
  ...
  const avgLatency = withLatency.reduce((sum, l) => sum + (l.latency_ms as number), 0) / withLatency.length
...
describe('analyzeUnmatchedPatterns', () => {
  ...
  it('finds patterns when 3+ inputs route to same AI intent', () => {
    const logs = [
      ...
      { detection_method: 'ai', detected_intent: 'task.create', input: 'create new task for me' },
    ]
  ...
  it('detects default note.create fallbacks as unmatched', () => {
    const logs = [
      ...
      { detection_method: 'rule', detected_intent: 'note.create', input: 'another random text' },
      { detection_method: 'rule', detected_intent: 'note.create', input: 'third random text' },
    ]
    const findings = analyzeUnmatchedPatterns(logs)
    expect(findings.length).toBe(1)
  })
...
describe('analyzeErrorClusters', () => {
  ...
  it('groups errors by domain+tool separately', () => {
    const logs = [
      ...Array.from({ length: 3 }, () => ({ domain: 'planning', tool_id: 'tasks', response: { success: false } })),
      ...Array.from({ length: 4 }, () => ({ domain: 'health', tool_id: 'tracker', response: { success: false } })),
    ]
    const findings = analyzeErrorClusters(logs)
