...
export interface TaskTypesState {
    taskTypes: TaskType[];
    isLoading: boolean;
    addTaskType: (t: Omit<TaskType, 'id' | 'createdAt' | 'sortOrder' | 'isPreset'> & { sortOrder?: number }) => Promise<TaskType>;
...
export const useTaskTypes = (): TaskTypesState => {
    ...
    const addTaskType = useCallback(async (t: Omit<TaskType, 'id' | 'createdAt' | 'sortOrder' | 'isPreset'> & { sortOrder?: number }) => {
    ...
    const updateTaskType = useCallback(async (t: TaskType) => {
        if (!userId) throw new Error('Not authenticated');
    ...
    const reorderTaskTypes = useCallback(async (ordered: string[]) => {
        if (!userId) throw new Error('Not authenticated');
        const updates = ordered.map((id, idx) =>
            supabase.from('task_types').update({ sort_order: idx }).eq('id', id).eq('user_id', userId)
        );
