 ...
 * triageOne() runs the same path for a single task right after capture.
 */
...
export function useTaskTriage(): UseTaskTriageReturn {
    ...
    // Untriaged, active capture inbox.
    const untriaged = useMemo(() => tasks.filter((t) => !t.completed && !t.triagedAt), [tasks]);
    ...
    // The review section: untriaged tasks the AI was NOT confident about. High-confidence
    // tasks are being auto-applied, so hide them here to avoid a flash before they leave.
    const reviewInbox = useMemo(() => {
        const highConfidence = new Set(
            (suggestions ?? []).filter((s) => s.confidence === 'high').map((s) => s.id),
        );
        return untriaged.filter((t) => !highConfidence.has(t.id));
