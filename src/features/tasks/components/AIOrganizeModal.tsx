...
import {
    getAIConfigStatus,
    organizeTasks,
    type TaskOrganizationSuggestion,
...
const AIOrganizeModal: React.FC<AIOrganizeModalProps> = ({
    candidates,
    taskTypes,
    onApply,
    onClose,
    ...
    const {
        ...
        queryFn: async () => {
            ...
            const result = await organizeTasks({
            ...
            return sanitizeOrganizeSuggestions(result, candidates, taskTypes);
        },
    });
                ...
                <div className="flex-1 overflow-y-auto p-5">
                    {!ready ? (
                        <div className="flex items-start gap-2 rounded-lg bg-amber-50 p-4 text-sm text-amber-700">
                            ...
                            Configure an AI provider in Account settings to organize tasks
                            automatically.
                        </div>
                    ) : isFetching ? (
                        ...
                        <div className="py-12 text-center text-sm text-slate-500">
                            Nothing to organize — every task already has a type and kind.
                        </div>
                    ) : (
                                ...
                                >
                                    ...
                                    <div className="flex flex-wrap items-center gap-2">
                                        <select
                                            ...
                                            onChange={(e) =>
                                                editRow(task.id, {
                                                    taskTypeId: e.target.value || null,
                                                })
                                            }
                                            disabled={isSkipped}
                                        ...
                                        <select
                                            ...
                                            onChange={(e) =>
                                                editRow(task.id, {
                                                    kind: e.target
                                                        .value as TaskOrganizationSuggestion['kind'],
                                                })
                                            }
                                            disabled={isSkipped}
                                            className="rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs text-slate-700"
                                        ...
                                        <select
                                            ...
                                            onChange={(e) =>
                                                editRow(task.id, {
                                                    priority: e.target
                                                        .value as TaskOrganizationSuggestion['priority'],
                                                })
                                            }
                                            disabled={isSkipped}
