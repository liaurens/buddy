...
const NoteSettingsModal: React.FC<NoteSettingsModalProps> = ({
  isOpen,
  onClose,
}) => {
  const { user } = useAuth();
  ...
  const updateSetting = <K extends keyof NotesSettings>(
    key: K,
    value: NotesSettings[K]
  ) => {
    if (settings) {
