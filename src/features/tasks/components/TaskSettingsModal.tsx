...
const TaskSettingsModal: React.FC<TaskSettingsModalProps> = ({ isOpen, onClose }) => {
    ...
    const updateSetting = <K extends keyof TaskSettings>(key: K, value: TaskSettings[K]) => {
        if (settings) {
    ...
    const footer = (
        <>
            <button
                onClick={handleReset}
                className="px-4 py-2 text-sm font-medium text-cove-muted hover:text-cove-ink transition-colors"
                disabled={saving}
            >
                Reset to Defaults
            </button>
            ...
            <button
                onClick={handleSave}
                className="px-4 py-2 text-sm font-medium text-white bg-cove-accent hover:bg-[#3a8dc7] rounded-md transition-colors disabled:opacity-50"
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Task Settings" footer={footer} size="lg">
            <div className="space-y-6">
                {/* Default Settings */}
                <div>
                    <h3 className="text-lg font-medium text-cove-ink mb-4">Defaults</h3>
                ...
                {/* Display Settings */}
                <div>
                    <h3 className="text-lg font-medium text-cove-ink mb-4">Display</h3>
                    <div className="space-y-4">
                ...
                <div>
                    <h3 className="text-lg font-medium text-cove-ink mb-4">Notifications</h3>
                    <div className="space-y-4">
                        {/* Enable Notifications */}
                        <div className="flex items-center justify-between">
                            <div>
                                <label className="text-sm font-medium text-cove-muted">
                ...
                {/* Archive Settings */}
                <div>
                    <h3 className="text-lg font-medium text-cove-ink mb-4">Archive</h3>
                    <div className="space-y-4">
                ...
                {/* Types & routines — harvested from the desktop-only tools page so
                    they are reachable on a phone (Me -> Tasks). */}
                <div>
                    <h3 className="mb-4 text-lg font-medium text-cove-ink">Task types</h3>
                    <TaskTypeManager />
                </div>

                <div>
                    <h3 className="mb-4 text-lg font-medium text-cove-ink">Routines</h3>
                    <p className="mb-3 text-sm text-cove-soft">
                        A routine is a batch of tasks you add to a day in one tap.
                    </p>
                    <RoutineEditor />
                </div>
            </div>
