            ...
            <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-2">
                        ...
                        <span className="w-32 text-center text-base font-semibold">
            ...
            <div className="app-surface overflow-hidden">
                <div className="grid grid-cols-7 border-b border-slate-100 bg-slate-50">
                    {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((d) => (
                        <div
                            key={d}
                            className="py-2 text-center text-xs font-bold text-slate-400 uppercase tracking-wider"
                        >
                            {d}
                        </div>
                    ))}
                ...
                <div className="grid grid-cols-7 text-sm">
                    {startPadding.map((_, i) => (
                        <div
                            key={`pad-${i}`}
                            className="h-24 border-b border-r border-slate-50 bg-slate-50/30 lg:h-32"
                        />
                    ))}
                    ...
                    {days.map((day) => {
                        ...
                        return (
                            <div
                                ...
                                className={`relative h-24 cursor-pointer border-b border-r border-slate-50 p-1 transition-colors lg:h-32 ${
                                    isSelected
                                        ? 'bg-indigo-50 ring-inset ring-2 ring-indigo-500'
                                        : 'hover:bg-slate-50'
                                }`}
                            >
                                <span
                                    className={`inline-block w-6 h-6 text-center leading-6 rounded-full font-medium text-xs ${
                                ...
                                <div className="mt-1 space-y-1">
                                    ...
                                    {assignmentChips.map((a) => {
                                        ...
                                        return (
                                            <div
                                                key={a.id}
                                                className={`text-[10px] truncate px-1 rounded flex items-center gap-1 ${isDone ? 'bg-slate-100 text-slate-400 line-through' : 'bg-rose-100 text-rose-700'}`}
                                            >
                                                <div
                                                    className="w-1 h-1 rounded-full flex-shrink-0"
                                                    style={{
                                                        backgroundColor: cls?.color ?? '#e11d48',
                                                    }}
                                                />
                                    ...
                                    {eventChips.map((event) => (
                                        <div
                                            key={event.id}
                                            className="text-[10px] truncate px-1 rounded flex items-center gap-1 bg-purple-100 text-purple-700"
                                        >
                                    ...
                                    {todoChips.map((todo) => (
                                        ...
                                        >
                                            <div
                                                className={`w-1 h-1 rounded-full ${todo.completed ? 'bg-slate-400' : 'bg-indigo-500'}`}
                                            />
                                            {todo.title}
                                        </div>
                                    ))}
                                    ...
                                    <div className="space-y-2">
                                        {getAssignmentsForDay(selectedDay).map((a) => {
                                            ...
                                            return (
                                                <div
                                                    key={a.id}
                                                    className={`flex items-start gap-3 p-3 rounded-lg border ${isDone ? 'bg-slate-50 border-slate-100' : 'bg-rose-50 border-rose-100'}`}
                                                >
                                                    <Clock
                                                        size={16}
                                                        className={`mt-0.5 shrink-0 ${isDone ? 'text-slate-400' : 'text-rose-600'}`}
                                                    />
                                                    <div className="flex-1 min-w-0">
                                                        <div
                                                            className={`font-medium truncate ${isDone ? 'text-slate-400 line-through' : 'text-slate-800'}`}
                                                        >
                                                            {a.title}
                                                        </div>
                                                        <div className="text-xs text-slate-500 mt-0.5">
