        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100">
            ...
            >
                <span className="flex items-center gap-2 text-sm font-semibold text-slate-700 uppercase tracking-wide">
                    <BookOpen size={16} className="text-amber-500" /> Journal — last {HISTORY_DAYS} days
                </span>
            ...
            {open && (
                <div className="px-5 pb-5">
                    {isLoading ? (
                        ...
                        <p className="text-sm text-rose-600 py-2">Could not load journal entries.</p>
                    ) : entries.length === 0 ? (
                            ...
                            {entries.map(entry => (
                                <li key={entry.id} className="border-l-2 border-amber-200 pl-3">
                                    ...
                                    <div className="mt-1 space-y-1.5">
                                        {entry.prompts.filter(p => p.answer).map(p => (
                                            <div key={p.promptId}>
                                                <span className="text-xs text-slate-400">{p.question}</span>
                                                <p className="text-sm text-slate-700 whitespace-pre-wrap">{p.answer}</p>
                                            </div>
                                        ))}
                                    </div>
                                </li>
