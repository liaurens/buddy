...
const SkillsLogCard: React.FC = () => {
    ...
    return (
        <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-100 space-y-4">
            ...
            <p className="text-xs text-slate-500 -mt-2">Minutes spent practicing each skill — XP is logged immediately.</p>
                ...
                {skills.map(skill => (
                    <li key={skill.id} className="flex items-center gap-3">
                        <span className="flex-1 min-w-0 text-sm font-medium text-slate-800 truncate">
                            ...
                            <span className="ml-2 text-xs font-normal text-slate-400">lvl {skill.level}</span>
                        </span>
                        {feedback?.skillId === skill.id ? (
                            <span className="text-xs font-medium text-emerald-600">{feedback.message}</span>
                        ) : (
                            <>
                                <input
                                    type="number"
                                    ...
                                    onChange={e => setMinutesBySkill(prev => ({ ...prev, [skill.id]: e.target.value }))}
                                ...
                                <button
                                    ...
                                    disabled={busySkillId === skill.id || !Number(minutesBySkill[skill.id])}
