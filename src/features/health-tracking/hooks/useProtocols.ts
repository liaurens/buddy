...
export function useProtocols(): ProtocolContextType {
    ...
    const addProtocol = useCallback(
        async (protocol: Omit<Protocol, 'id' | 'createdAt'>): Promise<string> => {
    ...
    const abortCycle = useCallback(
        async (cycleId: string, notes?: string) => {
            if (!userId) throw new Error('Not authenticated');
    ...
    const logDose = useCallback(
        async (protocolId: string, cycleId?: string, actualAmount?: number) => {
            if (!userId) throw new Error('Not authenticated');
    ...
    const skipDose = useCallback(
        async (protocolId: string, cycleId?: string, reason?: string) => {
            if (!userId) throw new Error('Not authenticated');
    ...
    const getActiveCycle = useCallback(
        (protocolId: string): Cycle | undefined => {
            return cycles.find((c) => c.protocolId === protocolId && c.status === 'active');
        },
        [cycles],
    );

    const getRecentDoses = useCallback(
        (protocolId: string, days: number = 7): Dose[] => {
            const cutoff = new Date();
            cutoff.setDate(cutoff.getDate() - days);

            return doses.filter(
                (d) => d.protocolId === protocolId && d.takenAt && new Date(d.takenAt) >= cutoff,
            );
        },
        [doses],
    );
