...
const STATUS_PILLS: Record<ExperimentStatus, { label: string; pill: string; strip: string }> = {
    active: { label: 'Active', pill: 'bg-green-50 text-green-700 border-green-100', strip: 'bg-indigo-500' },
    paused: { label: 'Paused', pill: 'bg-amber-50 text-amber-700 border-amber-100', strip: 'bg-amber-400' },
    completed: { label: 'Completed', pill: 'bg-indigo-50 text-indigo-700 border-indigo-100', strip: 'bg-indigo-300' },
    archived: { label: 'Archived', pill: 'bg-slate-50 text-slate-600 border-slate-100', strip: 'bg-slate-300' },
};
...
const ExperimentList: React.FC<ExperimentListProps> = ({ onRunAnalysis, onViewDetails, onCreateNew }) => {
    ...
    return (
        <div className="space-y-4">
                ...
                {(['all', 'active', 'paused', 'completed', 'archived'] as const).map(s => (
                    ...
                    >
                        ...
                        <span className="ml-1.5 opacity-75">
                            {s === 'all' ? experiments.length : experiments.filter(e => e.status === s).length}
                        </span>
                    </button>
                ))}
            </div>
            ...
            {filtered.length === 0 ? (
                <div className="text-center py-8 text-slate-400 text-sm">No experiments in this category.</div>
            ) : filtered.map(ex => {
                ...
                return (
                    <div key={ex.id} className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden">
                        <div className={`absolute top-0 left-0 w-1 h-full ${statusInfo.strip}`}></div>
                        ...
                        <div className="flex justify-between items-start mb-3 pl-2">
                            <div className="flex-1 min-w-0">
                                ...
                                {ex.hypothesis && (
                                    <p className="text-sm text-slate-600 mt-0.5 line-clamp-1">{ex.hypothesis}</p>
                                )}
                                <div className="flex items-center gap-2 text-xs text-slate-500 mt-1.5 flex-wrap">
                                    <span className={`px-2 py-0.5 rounded-full border ${statusInfo.pill}`}>
                                        {statusInfo.label}
                                    </span>
                            ...
                            <button
                                onClick={() => {
                                    if (window.confirm('Delete this experiment? Check-in data will be preserved but the experiment definition will be removed.')) {
                        ...
                        {hasVariables && (
                            <div className="flex items-center gap-3 mb-4 px-2 cursor-pointer" onClick={() => onViewDetails(ex)}>
                                <div className="flex-1 bg-slate-50 p-2.5 rounded-lg text-center border border-slate-100 flex flex-col items-center justify-center gap-1">
                                    {independentVars.slice(0, 3).map((v, i) => (
                                        <div key={i} className="flex items-center gap-1.5 w-full justify-center">
                                            ...
                                            <span className="text-xs font-medium text-slate-700 truncate max-w-[100px]">{v.name}</span>
                                            {i < independentVars.length - 1 && i < 2 && <span className="text-slate-300">+</span>}
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="flex-1 bg-slate-50 p-2.5 rounded-lg text-center border border-slate-100">
                                    <span className="text-base block">{dependentVar.emoji}</span>
                                    <span className="text-xs font-medium text-slate-700 truncate block">{dependentVar.name}</span>
                                </div>
                            </div>
                        )}
