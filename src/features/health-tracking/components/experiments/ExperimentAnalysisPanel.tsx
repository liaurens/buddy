...
const ExperimentAnalysisPanel: React.FC<ExperimentAnalysisPanelProps> = ({
    ...
    const valuesFor = (metricId: string, phaseId: string): number[] =>
        checkins
            .filter(c => c.metricId === metricId && c.phaseId === phaseId && c.value !== undefined)
            .map(c => c.value!);
    ...
    return (
        <div className="space-y-6">
            ...
            {overallStats.filter(s => s.n > 0).map(({ metric, mean, std, n, min, max, trend }) => (
                <div key={metric.id} className="bg-slate-50 rounded-xl p-4 space-y-2">
                    ...
                    <div className="flex items-baseline gap-4">
                        <div className="text-2xl font-bold text-indigo-600">{mean.toFixed(1)}</div>
            ...
            {canCompare && (
                <div className="space-y-3">
                    ...
                    {numericMetrics.map(metric => {
                        ...
                        return (
                            <div key={metric.id} className="bg-white border border-slate-100 rounded-xl p-4 space-y-3">
                                <div className="flex items-center justify-between">
                                    <span className="text-sm font-medium text-slate-700">
                                        {metric.emoji} {metric.name}
                                    </span>
                                    <span className="text-xs text-slate-400">
                                        baseline {baseStats.mean.toFixed(1)} ± {baseStats.std.toFixed(1)} (n={baseStats.n})
                                    </span>
                                </div>
                                ...
                                {comparisonPhases.map(phase => {
                                    ...
                                    return (
                                        <div key={phase.id} className="border-t border-slate-100 pt-3 space-y-1.5">
                                            <div className="flex items-center justify-between">
                                                <span className="text-xs font-medium text-slate-600">vs {phase.name}</span>
                                                <span className="text-xs text-slate-400">
                                                    {intStats.mean.toFixed(1)} ± {intStats.std.toFixed(1)} (n={intStats.n})
                                                </span>
                                            </div>
                                            ...
                                            {sparse ? (
                                                <div className="flex items-start gap-2 text-xs text-amber-700 bg-amber-50 border border-amber-100 rounded-lg px-2.5 py-1.5">
                                                    ...
                                                    <span>
                                                        Need at least {MIN_N} entries per phase for reliable stats
                                                        (baseline={baseStats.n}, intervention={intStats.n}).
                                                    </span>
                                                </div>
                                            ) : welch && d && ci ? (
            ...
            {!canCompare && phases.length > 1 && (
                <div className="space-y-3">
                    ...
                    <p className="text-xs text-slate-500">
                        Mark one phase as baseline in the wizard to unlock Welch's t, CI, and effect size.
                    </p>
                    {numericMetrics.map(metric => (
