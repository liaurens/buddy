 ...
 */
export async function getUpcomingNotifications(userId: string): Promise<ScheduledNotification[]> {
...
export async function scheduleProtocolDose(
    ...
    return scheduleNotificationAt(userId, 'protocol', 'protocol_dose', scheduledFor, title, body, {
        protocolName,
        doseTime,
    });
}
...
export async function scheduleCheckInReminder(
    ...
    return scheduleDailyNotification(userId, 'checkin', 'checkin_daily', reminderTime, title, body);
}
...
export async function scheduleTaskDue(
    ...
    return scheduleNotificationAt(userId, 'tasks', 'task_due', scheduledFor, title, body, {
        taskTitle,
...
export function computeTaskReminderFireTimes(
...
): Array<{ at: Date; type: 'pre' | 'at' | 'late'; lateMinutes?: number }> {
    ...
    if (cadence === 'smart') {
        fires.push({
            at: new Date(input.dueAt.getTime() + 15 * 60_000),
            type: 'late',
            lateMinutes: 15,
        });
        fires.push({
            at: new Date(input.dueAt.getTime() + 60 * 60_000),
            type: 'late',
            lateMinutes: 60,
        });
    } else {
        ...
        fires.push({
            at: new Date(input.dueAt.getTime() + 30 * 60_000),
            type: 'late',
            lateMinutes: 30,
        });
        fires.push({
        ...
        fires.push({
            at: new Date(input.dueAt.getTime() + 120 * 60_000),
            type: 'late',
            lateMinutes: 120,
        });
    }
...
export async function schedulePomodoroComplete(
    ...
    return scheduleNotificationIn(userId, 'pomodoro', 'pomodoro_complete', minutes, title, body, {
        sessionType,
    });
}
...
export async function scheduleCalendarEvent(
    ...
    const scheduledFor = new Date(eventStart.getTime() - advanceMinutes * 60 * 1000);

    return scheduleNotificationAt(userId, 'calendar', 'calendar_event', scheduledFor, title, body, {
        eventTitle,
