...
export async function updateCategorySettings<T extends SettingCategory>(
    ...
    const validated = schema.partial().parse(updates) as Record<string, unknown>;
 ...
 */
export async function getSingleSetting<T extends SettingCategory, K extends keyof AllSettings[T]>(
    userId: string,
    category: T,
    key: K,
...
>(userId: string, category: T, key: K, value: AllSettings[T][K]): Promise<void> {
    await updateCategorySettings(userId, category, { [key]: value } as unknown as Partial<
        AllSettings[T]
    >);
}
 ...
 */
export function getDefaultSettings<T extends SettingCategory>(category: T): AllSettings[T] {
    const schema = settingsSchemas[category];
